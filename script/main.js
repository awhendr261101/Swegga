...
document.querySelectorAll(".entry").forEach((value,index) => value.addEventListener("change", ()=>{
    if(index == 0){
        ...
        list[index].textContent = `R${(eval(value.value * 150.95)).toFixed(2)}`
    } else {
        num2 = eval(value.value * 180.95)
        list[index].textContent = `R${(eval(value.value * 180.95)).toFixed(2)}`
    }
}))


document.querySelector(".total").addEventListener("click", ()=>{
    list[2].textContent = `R${(num1 + num2).toFixed(2)}`
})
